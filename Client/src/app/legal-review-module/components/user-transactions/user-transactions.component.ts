...
export class UserTransactionsComponent implements OnInit {
  ...
  getForwadedLoans(): any {
    this.filteredLoans = [];
    this.loanTable = [];
    this.userTransactions.getForwardedLegalReviewLoans(this.User.branchId).subscribe((userData) => {
  ...
  getReceivedLoans(): any {
    this.filteredLoans = [];
    this.loanTable = [];
    this.userTransactions.getReceivedlegalReviewLoans(this.User.branchId).subscribe((userData) => {
      this.loanTable = userData.map((eachUser) => {
        const oldDate = eachUser.CreatedAt;
        const diffInDates = moment(this.age).diff(moment(oldDate));
        const timeInMonths = moment(diffInDates).format(
  ...
  getApprovedLoans(): any {
    this.filteredLoans = [];
    this.loanTable = [];
    this.userTransactions.getApprovedlegalReviewLoans(this.User.branchId).subscribe((userData) => {
      this.loanTable = userData.map((eachUser) => {
        const oldDate = eachUser.CreatedAt;
        const diffInDates = moment(this.age).diff(moment(oldDate));
        const timeInMonths = moment(diffInDates).format(
  ...
  receive(loan: any, category: string): any{
    ...
    this.userTransactions.receiveForwardedlegalReviewLoans(data).subscribe(
      res => {
        this.posted = true;
        this.getForwadedLoans();
        this.spinner.hide();
  ...
  approveLoan(data: any): any {
    this.spinner.show();
    this.userTransactions.approvelegalReviewLoans(data).subscribe(
      res => {
        this.posted = true;
  ...
  deferLoan(data: any): any {
    this.spinner.show();
    this.userTransactions.deferlegalReviewLoans(data).subscribe(
      res => {
        this.posted = true;
        this.getReceivedLoans();
        this.spinner.hide();
  ...
  forwadLoan(data: any): any {
    this.spinner.show();
    this.userTransactions.forwardlegalReviewLoans(data).subscribe(
      res => {
        this.posted = true;
