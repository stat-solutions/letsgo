...
export class UserTransactionsComponent implements OnInit {
  ...
  getForwadedLoans(): any {
    this.filteredLoans = [];
    this.loanTable = [];
    this.userTransactions.getForwardedLoanDisbursementLoans(this.User.branchId).subscribe((userData) => {
  ...
  getReceivedLoans(): any {
    this.filteredLoans = [];
    this.loanTable = [];
    this.userTransactions.getReceivedLoanDisbursementLoans(this.User.branchId).subscribe((userData) => {
      this.loanTable = userData.map((eachUser) => {
        const oldDate = eachUser.CreatedAt;
        const diffInDates = moment(this.age).diff(moment(oldDate));
        const timeInMonths = moment(diffInDates).format(
  ...
  getApprovedLoans(): any {
    this.filteredLoans = [];
    this.loanTable = [];
    this.userTransactions.getApprovedLoanDisbursementLoans(this.User.branchId).subscribe((userData) => {
      this.loanTable = userData.map((eachUser) => {
        const oldDate = eachUser.CreatedAt;
        const diffInDates = moment(this.age).diff(moment(oldDate));
        const timeInMonths = moment(diffInDates).format(
  ...
  receive(loan: any, category: string): any{
    ...
    this.userTransactions.receiveForwardedLoanDisbursementLoans(data).subscribe(
      res => {
        this.posted = true;
        this.getForwadedLoans();
        this.spinner.hide();
  ...
  approveLoan(data: any): any {
    this.spinner.show();
    this.userTransactions.approveLoanDisbursementLoans(data).subscribe(
      res => {
        this.posted = true;
  ...
  deferLoan(data: any): any {
    this.spinner.show();
    this.userTransactions.deferLoanDisbursementLoans(data).subscribe(
      res => {
        this.posted = true;
        this.getReceivedLoans();
        this.spinner.hide();
  ...
  disburseLoan(data: any): any {
    this.spinner.show();
    this.userTransactions.disburseLoanDisbursementLoans(data).subscribe(
      res => {
        this.posted = true;
        this.getReceivedLoans();
        this.spinner.hide();
        this.alertService.success({
          html: '<b>Loan was disbursed successfully</b>',
        });
      },
      err => {
        this.errored = true;
